  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <ReactMarkdown
        ...
        components={{
          h1: ({ children }) => (
            <h1 className="text-3xl font-bold text-foreground mt-10 mb-4 first:mt-0" id={String(children).toLowerCase().replace(/\s+/g, "-")}>
              {children}
            </h1>
          ),
          h2: ({ children }) => (
            <h2 className="text-2xl font-semibold text-foreground mt-8 mb-3" id={String(children).toLowerCase().replace(/\s+/g, "-")}>
              {children}
            </h2>
          ),
          h3: ({ children }) => (
            <h3 className="text-lg font-semibold text-foreground mt-6 mb-2" id={String(children).toLowerCase().replace(/\s+/g, "-")}>
              {children}
            </h3>
          ),
          p: ({ children }) => (
          ...
          pre: ({ children }) => (
            <pre className="bg-card border border-border rounded-lg p-4 overflow-x-auto my-4 text-sm">{children}</pre>
          ),
          table: ({ children }) => (
